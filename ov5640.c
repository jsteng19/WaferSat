/*
 * Registers by Arducam https://github.com/ArduCAM/Arduino/blob/master/ArduCAM/ov5640_regs.h
 * https://github.com/ArduCAM/Arduino/blob/master/ArduCAM/ArduCAM.cpp
 */

#include "ch.h"
#include "hal.h"
#include "ov5640.h"
#include "pi2c.h"
#include "board.h"
#include <string.h>
#include "dcmi_dma.h"
#include "chprintf.h"
#include "stm32f4xx_rcc.h"
#include "stm32f4xx_dcmi.h"
#include "sd.h"

// create buffer in DRAM
#define IN_DRAM __attribute__((section(".ram7")))
uint8_t OV5640_ram_buffer[OV5640_NUM_PIXELS*2] IN_DRAM;
volatile uint32_t OV5640_ram_buffer_length = 0;
 
static uint32_t lightIntensity;
static uint8_t error;
volatile bool samplingFinished;
bool    decode_pause;

struct regval_list {
	uint16_t reg;
	uint8_t val;
};

const uint8_t noCameraFound[] = {
     
};


static const struct regval_list OV5640YUV_Sensor_Dvp_Init[] =
{
 //reset and power down camera for configuration
 { 0x3008, 0x82 }, //software reset
 { 0x3008, 0x42 }, //exit reset into power down
 //clock configuration - ~16MHz PCLK, ~5Hz image rate at 5MP
 { 0x3034, 0x1a }, //default PLL charge pump control
 { 0x3035, 0x31 }, //divide all clocks by 3
 { 0x3036, 0xB4 }, //PLL multiplier = 180
                   //can be any integer 4-127, any even 128-252
 { 0x3037, 0x13 }, //pll root-divider = 2, pll pre-divider=3
 { 0x3039, 0x00 }, //default disable PLL bypass
 { 0x3103, 0x02 }, //system clock from PLL
 { 0x3108, 0x01 }, //no PCLK root divider
 { 0x460c, 0x22 }, //PCLK divider controlled by 0x3824
 { 0x3824, 0x04 }, //divide PCLK by 4
 { 0x3002, 0x00 }, //enable all reset block 2
 { 0x3006, 0xff }, //enable all clock block 2
 //sync configuration
 { 0x4740, 0x24 }, //VSYNC active high, HREF active high
 //gate PCKL under HREF, PCLK active high
 { 0x3017, 0x7f }, //enable all clock outputs
 { 0x3018, 0xff }, //enable all data outputs
 { 0x302c, 0x02 }, //default FREX enable
 { 0x4713, 0x03 }, // JPEG mode 3 clock signals
 { 0x4404, 0x34 }, // Gated Clock Enabled (default 0x24)
 //format configuration
 { 0x3821, 0x20 }, //JPEG compression enable
 { 0x4300, 0xF8 }, //default bypass formatter, raw output
 { 0x501f, 0x00 }, //default format mux YUV422
 //{0x503D, 0x80}, //test mode 8 color bar vertical

  //AEC
 { 0x3a02, 0x07 }, //AEC 60Hz max exposure H
 { 0x3a03, 0xb0 }, //AEC 60Hz max exposure L = 1968
 { 0x3a0e, 0x06 }, //AEC 50Hz max bands in one frame = 6
 { 0x3a0d, 0x08 }, //AEC 60Hz max bands in one frame = 8
 { 0x3a14, 0x07 }, //AEC 50hz max exposure H
 { 0x3a15, 0xb0 }, //AEC 50Hz max exposure L = 1968
 //        //50/60Hz detector control
 //        { 0x3c04, 0x28 },
 //        { 0x3c05, 0x98 },
 //        { 0x3c06, 0x00 },
 //        { 0x3c07, 0x08 },
 //        { 0x3c08, 0x00 },
 //        { 0x3c09, 0x1c },
 //        { 0x3c0a, 0x9c },
 //        { 0x3c0b, 0x40 },
 //
 //        //50/60Hz band removal
 //        { 0x3c01, 0xb4 },
 //        { 0x3c00, 0x04 },
 //        { 0x3a08, 0x00 },
 //        { 0x3a09, 0x93 },
 //        { 0x3a0e, 0x06 },
 //        { 0x3a0a, 0x00 },
 //        { 0x3a0b, 0x7b },
 //        { 0x3a0d, 0x08 },
 //
 //        //AEC/AGC control
 //        { 0x3a18, 0x00 }, //AEC gain ceiling H
 //        { 0x3a19, 0xf8 }, //AEC gain ceiling L = 248

 //        { 0x3a00, 0x3c }, //15fps-10fps
 //                          //enable night mode
 //                          //enable less-than-one-line exposure
 //        { 0x3a02, 0x05 }, //60hz night mode, max exposure H
 //        { 0x3a03, 0xc4 }, //max exposure L = 1476
 //        { 0x3a14, 0x05 }, //50hz night mode, max exposure H
 //        { 0x3a15, 0xc4 }, //max exposure L = 1476
 //

 //BLC control
 { 0x4001, 0x02 }, //BLC start line = 2
 { 0x4004, 0x06 }, //BLC line number = 6

 //AWB gain settings
 { 0x3406, 0x01 },//awb manual control
 { 0x3400, 0x06 },
 { 0x3401, 0x80 },
 { 0x3402, 0x04 },
 { 0x3403, 0x00 },
 { 0x3404, 0x06 },
 { 0x3405, 0x00 },
 //AWB control
 { 0x5180, 0xff },
 { 0x5181, 0xf2 },
 { 0x5182, 0x00 },
 { 0x5183, 0x14 },
 { 0x5184, 0x25 },
 { 0x5185, 0x24 },
 { 0x5186, 0x16 },
 { 0x5187, 0x16 },
 { 0x5188, 0x16 },
 { 0x5189, 0x62 },
 { 0x518a, 0x62 },
 { 0x518b, 0xf0 },
 { 0x518c, 0xb2 },
 { 0x518d, 0x50 },
 { 0x518e, 0x30 },
 { 0x518f, 0x30 },
 { 0x5190, 0x50 },
 { 0x5191, 0xf8 },
 { 0x5192, 0x04 },
 { 0x5193, 0x70 },
 { 0x5194, 0xf0 },
 { 0x5195, 0xf0 },
 { 0x5196, 0x03 },
 { 0x5197, 0x01 },
 { 0x5198, 0x04 },
 { 0x5199, 0x12 },
 { 0x519a, 0x04 },
 { 0x519b, 0x00 },
 { 0x519c, 0x06 },
 { 0x519d, 0x82 },
 { 0x519e, 0x38 },
 //color matrix
 { 0x5381, 0x1e },
 { 0x5382, 0x5b },
 { 0x5383, 0x14 },
 { 0x5384, 0x06 },
 { 0x5385, 0x82 },
 { 0x5386, 0x88 },
 { 0x5387, 0x7c },
 { 0x5388, 0x60 },
 { 0x5389, 0x1c },
 { 0x538a, 0x01 },
 { 0x538b, 0x98 },
 //sharp&noise
 { 0x5300, 0x08 },
 { 0x5301, 0x30 },
 { 0x5302, 0x3f },
 { 0x5303, 0x10 },
 { 0x5304, 0x08 },
 { 0x5305, 0x30 },
 { 0x5306, 0x18 },
 { 0x5307, 0x28 },
 { 0x5309, 0x08 },
 { 0x530a, 0x30 },
 { 0x530b, 0x04 },
 { 0x530c, 0x06 },
 //gamma
 { 0x5480, 0x01 },
 { 0x5481, 0x06 },
 { 0x5482, 0x12 },
 { 0x5483, 0x24 },
 { 0x5484, 0x4a },
 { 0x5485, 0x58 },
 { 0x5486, 0x65 },
 { 0x5487, 0x72 },
 { 0x5488, 0x7d },
 { 0x5489, 0x88 },
 { 0x548a, 0x92 },
 { 0x548b, 0xa3 },
 { 0x548c, 0xb2 },
 { 0x548d, 0xc8 },
 { 0x548e, 0xdd },
 { 0x548f, 0xf0 },
 { 0x5490, 0x15 },
 // //UV adjust - not enabled, does not change image
 // { 0x5580, 0x06 },
 // { 0x5583, 0x40 },
 // { 0x5584, 0x20 },
 // { 0x5589, 0x10 },
 // { 0x558a, 0x00 },
 // { 0x558b, 0xf8 },
 //lens shading
 { 0x5000, 0xa7 },
 { 0x5800, 0x20 },
 { 0x5801, 0x19 },
 { 0x5802, 0x17 },
 { 0x5803, 0x16 },
 { 0x5804, 0x18 },
 { 0x5805, 0x21 },
 { 0x5806, 0x0F },
 { 0x5807, 0x0A },
 { 0x5808, 0x07 },
 { 0x5809, 0x07 },
 { 0x580a, 0x0A },
 { 0x580b, 0x0C },
 { 0x580c, 0x0A },
 { 0x580d, 0x03 },
 { 0x580e, 0x01 },
 { 0x580f, 0x01 },
 { 0x5810, 0x03 },
 { 0x5811, 0x09 },
 { 0x5812, 0x0A },
 { 0x5813, 0x03 },
 { 0x5814, 0x01 },
 { 0x5815, 0x01 },
 { 0x5816, 0x03 },
 { 0x5817, 0x08 },
 { 0x5818, 0x10 },
 { 0x5819, 0x0A },
 { 0x581a, 0x06 },
 { 0x581b, 0x06 },
 { 0x581c, 0x08 },
 { 0x581d, 0x0E },
 { 0x581e, 0x22 },
 { 0x581f, 0x18 },
 { 0x5820, 0x13 },
 { 0x5821, 0x12 },
 { 0x5822, 0x16 },
 { 0x5823, 0x1E },
 { 0x5824, 0x64 },
 { 0x5825, 0x2A },
 { 0x5826, 0x2C },
 { 0x5827, 0x2A },
 { 0x5828, 0x46 },
 { 0x5829, 0x2A },
 { 0x582a, 0x26 },
 { 0x582b, 0x24 },
 { 0x582c, 0x26 },
 { 0x582d, 0x2A },
 { 0x582e, 0x28 },
 { 0x582f, 0x42 },
 { 0x5830, 0x40 },
 { 0x5831, 0x42 },
 { 0x5832, 0x08 },
 { 0x5833, 0x28 },
 { 0x5834, 0x26 },
 { 0x5835, 0x24 },
 { 0x5836, 0x26 },
 { 0x5837, 0x2A },
 { 0x5838, 0x44 },
 { 0x5839, 0x4A },
 { 0x583a, 0x2C },
 { 0x583b, 0x2a },
 { 0x583c, 0x46 },
 { 0x583d, 0xCE },
 //AEC AVG settings
 { 0x5688, 0x22 },
 { 0x5689, 0x22 },
 { 0x568a, 0x42 },
 { 0x568b, 0x24 },
 { 0x568c, 0x42 },
 { 0x568d, 0x24 },
 { 0x568e, 0x22 },
 { 0x568f, 0x22 },
 //AEC settings
 { 0x3a0f, 0x30 },
 { 0x3a10, 0x28 },
 { 0x3a1b, 0x30 },
 { 0x3a1e, 0x28 },
 { 0x3a11, 0x61 },
 { 0x3a1f, 0x10 },
 //Binning
 { 0x3814, 0x11 }, // --x inc, no binning
 { 0x3815, 0x11 }, // --y inc, no binning
 //Scaling
 { 0x5001, 0x83 }, //disable scaler
 //Mirror and flip
 { 0x3820, 0x46 }, //V flip
 { 0x3821, 0x20 }, //no H mirror, no horizontal binning, enable JPEG
 //Image size config QSXGA (2592x1944)
 { 0x3800, 0x00 }, //x start 0
 { 0x3801, 0x00 },
 { 0x3802, 0x00 }, //y start 0
 { 0x3803, 0x00 },
 { 0x3804, 0x0A }, //x end 2623
 { 0x3805, 0x3f },
 { 0x3806, 0x07 }, //y end 1951
 { 0x3807, 0x9f },
 { 0x3808, 0x0a }, //output width 2952
 { 0x3809, 0x20 },
 { 0x380a, 0x07 }, //output height 1944
 { 0x380b, 0x98 },
 { 0x380c, 0x0b }, //total H size 2844
 { 0x380d, 0x1c },
 { 0x380e, 0x07 }, //total V size 1968
 { 0x380f, 0xB0 },
 { 0x5680, 0x0  }, //AVG window X start 0
 { 0x5681, 0x0  },
 { 0x5682, 0xA  }, //AVG window Y start 0
 { 0x5683, 0x20 },
 { 0x5684, 0x0  }, //AVG window X width 0
 { 0x5685, 0x0  },
 { 0x5686, 0x7  }, //AVG window Y width 1944
 { 0x5687, 0x98 },
 //Start up camera
 { 0x4005, 0x1a }, //BLC mode always update
 { 0x3406, 0x00 }, //AWB mode auto
 { 0x3503, 0x00 }, //AEC mode auto, AGC mode auto
 { 0x3008, 0x02 }, //exit software power down

 { 0xffff, 0xff } // end
};


/*
 * TODO: This resolution configuration is currently used as the JPEG setup.
 * There should be a generic JPEG setup.
 */
//2592x1944 QSXGA
static const struct regval_list OV5640_JPEG_QSXGA[] __attribute__((unused)) =
{
 //Binning
 { 0x3814, 0x11 }, // --x inc, no binning
 { 0x3815, 0x11 }, // --y inc, no binning
 //Scaling
 { 0x5001, 0x83 }, //disable scaler
 //Image size config QSXGA (2592x1944)
 { 0x3800, 0x00 }, //x start 0
 { 0x3801, 0x00 },
 { 0x3802, 0x00 }, //y start 0
 { 0x3803, 0x00 },
 { 0x3804, 0x0A }, //x end 2623
 { 0x3805, 0x3f },
 { 0x3806, 0x07 }, //y end 1951
 { 0x3807, 0x9f },
 { 0x3808, 0x0a }, //output width 2952
 { 0x3809, 0x20 },
 { 0x380a, 0x07 }, //output height 1944
 { 0x380b, 0x98 },
 { 0x380c, 0x0b }, //total H size 2844
 { 0x380d, 0x1c },
 { 0x380e, 0x07 }, //total V size 1968
 { 0x380f, 0xB0 },
 { 0x5680, 0x0  }, //AVG window X start 0
 { 0x5681, 0x0  },
 { 0x5682, 0xA  }, //AVG window Y start 0
 { 0x5683, 0x20 },
 { 0x5684, 0x0  }, //AVG window X width 0
 { 0x5685, 0x0  },
 { 0x5686, 0x7  }, //AVG window Y width 1944
 { 0x5687, 0x98 },
};

//5MP -- same as QSXGA
static const struct regval_list OV5640_5MP_JPEG[] __attribute__((unused)) =
{
 //Binning
 { 0x3814, 0x11 }, // --x inc, no binning
 { 0x3815, 0x11 }, // --y inc, no binning
 //Scaling
 { 0x5001, 0x83 }, //disable scaler
 //Image size config QSXGA (2592x1944)
 { 0x3800, 0x00 }, //x start 0
 { 0x3801, 0x00 },
 { 0x3802, 0x00 }, //y start 0
 { 0x3803, 0x00 },
 { 0x3804, 0x0A }, //x end 2623
 { 0x3805, 0x3f },
 { 0x3806, 0x07 }, //y end 1951
 { 0x3807, 0x9f },
 { 0x3808, 0x0a }, //output width 2952
 { 0x3809, 0x20 },
 { 0x380a, 0x07 }, //output height 1944
 { 0x380b, 0x98 },
 { 0x380c, 0x0b }, //total H size 2844
 { 0x380d, 0x1c },
 { 0x380e, 0x07 }, //total V size 1968
 { 0x380f, 0xB0 },
 { 0x5680, 0x0  }, //AVG window X start 0
 { 0x5681, 0x0  },
 { 0x5682, 0xA  }, //AVG window Y start 0
 { 0x5683, 0x20 },
 { 0x5684, 0x0  }, //AVG window X width 0
 { 0x5685, 0x0  },
 { 0x5686, 0x7  }, //AVG window Y width 1944
 { 0x5687, 0x98 },
 { 0xffff, 0xff }, //end
};

//320x240 QVGA
static const struct regval_list OV5640_QSXGA2QVGA[] __attribute__((unused)) =
{
    {0x3800 ,0x00},
    {0x3801 ,0x00},
    {0x3802 ,0x00},
    {0x3803 ,0x00},
    {0x3804 ,0xA },
    {0x3805 ,0x3f},
    {0x3806 ,0x7 },
    {0x3807 ,0x9f},
    {0x3808 ,0x1 },
    {0x3809 ,0x40},
    {0x380a ,0x0 },
    {0x380b ,0xf0},
    {0x380c ,0xc },
    {0x380d ,0x80},
    {0x380e ,0x7 },
    {0x380f ,0xd0},
    {0x5001 ,0xa3},
    {0x5680 ,0x0 },
    {0x5681 ,0x0 },
    {0x5682 ,0xA },
    {0x5683 ,0x20},
    {0x5684 ,0x0 },
    {0x5685 ,0x0 },
    {0x5686 ,0x7 },
    {0x5687 ,0x98},
    {0xffff, 0xff},
};

//320x240 QQVGA
static const struct regval_list OV5640_QSXGA2QQVGA[] __attribute__((unused)) =
{
    {0x3800 ,0x00},
    {0x3801 ,0x00},
    {0x3802 ,0x00},
    {0x3803 ,0x00},
    {0x3804 ,0xA },
    {0x3805 ,0x3f},
    {0x3806 ,0x7 },
    {0x3807 ,0x9f},
    {0x3808 ,0x0 },
    {0x3809 ,0xA0},
    {0x380a ,0x0 },
    {0x380b ,0x70},
    {0x380c ,0xc },
    {0x380d ,0x80},
    {0x380e ,0x7 },
    {0x380f ,0xd0},
    {0x5001 ,0xa3},
    {0x5680 ,0x0 },
    {0x5681 ,0x0 },
    {0x5682 ,0xA },
    {0x5683 ,0x20},
    {0x5684 ,0x0 },
    {0x5685 ,0x0 },
    {0x5686 ,0x7 },
    {0x5687 ,0x98},
    {0xffff, 0xff},
};

//640x480 VGA
static const struct regval_list OV5640_QSXGA2VGA[] __attribute__((unused)) =
{
        {0x3800 ,0x00},
        {0x3801 ,0x00},
        {0x3802 ,0x00},
        {0x3803 ,0x00},
        {0x3804 ,0xA },
        {0x3805 ,0x3f},
        {0x3806 ,0x7 },
        {0x3807 ,0x9f},
        {0x3808 ,0x2 },
        {0x3809 ,0x80},
        {0x380a ,0x1 },
        {0x380b ,0xe0},
        {0x380c ,0xc },
        {0x380d ,0x80},
        {0x380e ,0x7 },
        {0x380f ,0xd0},
        {0x5001 ,0xa3},
        {0x5680 ,0x0 },
        {0x5681 ,0x0 },
        {0x5682 ,0xA },
        {0x5683 ,0x20},
        {0x5684 ,0x0 },
        {0x5685 ,0x0 },
        {0x5686 ,0x7 },
        {0x5687 ,0x98},
        {0xffff, 0xff},
};

//800x480 WVGA
static const struct regval_list OV5640_QSXGA2WVGA[] __attribute__((unused)) =
{
    {0x3800 ,0x00},
    {0x3801 ,0x00},
    {0x3802 ,0x00},
    {0x3803 ,0x00},
    {0x3804 ,0xA },
    {0x3805 ,0x3f},
    {0x3806 ,0x7 },
    {0x3807 ,0x9f},
    {0x3808 ,0x3 },
    {0x3809 ,0x20},
    {0x380a ,0x1 },
    {0x380b ,0xe0},
    {0x380c ,0xc },
    {0x380d ,0x80},
    {0x380e ,0x7 },
    {0x380f ,0xd0},
    {0x3810, 0x00},
    {0x3811, 0x10},
    {0x3812, 0x01},
    {0x3813, 0x48},
    {0x5001 ,0xa3},
    {0x5680 ,0x0 },
    {0x5681 ,0x0 },
    {0x5682 ,0xA },
    {0x5683 ,0x20},
    {0x5684 ,0x0 },
    {0x5685 ,0x0 },
    {0x5686 ,0x7 },
    {0x5687 ,0x98},
    {0xffff, 0xff},
};

//352x288 CIF
static const struct regval_list OV5640_QSXGA2CIF[] __attribute__((unused)) =
{
    {0x3800 ,0x00},
    {0x3801 ,0x00},
    {0x3802 ,0x00},
    {0x3803 ,0x00},
    {0x3804 ,0xA },
    {0x3805 ,0x3f},
    {0x3806 ,0x7 },
    {0x3807 ,0x9f},
    {0x3808 ,0x1 },
    {0x3809 ,0x60},
    {0x380a ,0x1 },
    {0x380b ,0x20},
    {0x380c ,0xc },
    {0x380d ,0x80},
    {0x380e ,0x7 },
    {0x380f ,0xd0},
    {0x3810, 0x00},
    {0x3811, 0x10},
    {0x3812, 0x00},
    {0x3813, 0x70},
    {0x5001 ,0xa3},
    {0x5680 ,0x0 },
    {0x5681 ,0x0 },
    {0x5682 ,0xA },
    {0x5683 ,0x20},
    {0x5684 ,0x0 },
    {0x5685 ,0x0 },
    {0x5686 ,0x7 },
    {0x5687 ,0x98},
    {0xffff, 0xff},
};

//1280x960 SXGA
static const struct regval_list OV5640_QSXGA2SXGA[] __attribute__((unused)) =
{
    {0x3800 ,0x00},
    {0x3801 ,0x00},
    {0x3802 ,0x00},
    {0x3803 ,0x00},
    {0x3804 ,0xA },
    {0x3805 ,0x3f},
    {0x3806 ,0x7 },
    {0x3807 ,0x9f},
    {0x3808 ,0x5 },
    {0x3809 ,0x0 },
    {0x380a ,0x3 },
    {0x380b ,0xc0},
    {0x380c ,0xc },
    {0x380d ,0x80},
    {0x380e ,0x7 },
    {0x380f ,0xd0},
    {0x5001 ,0xa3},
    {0x5680 ,0x0 },
    {0x5681 ,0x0 },
    {0x5682 ,0xA },
    {0x5683 ,0x20},
    {0x5684 ,0x0 },
    {0x5685 ,0x0 },
    {0x5686 ,0x7 },
    {0x5687 ,0x98},
    {0xffff, 0xff},
};

//2048x1536 QXGA
static const struct regval_list OV5640_QSXGA2QXGA[] __attribute__((unused)) =
{
    {0x3800 ,0x00},
    {0x3801 ,0x00},
    {0x3802 ,0x00},
    {0x3803 ,0x00},
    {0x3804 ,0xA },
    {0x3805 ,0x3f},
    {0x3806 ,0x7 },
    {0x3807 ,0x9f},
    {0x3808 ,0x8 },
    {0x3809 ,0x0 },
    {0x380a ,0x6 },
    {0x380b ,0x0 },
    {0x380c ,0xc },
    {0x380d ,0x80},
    {0x380e ,0x7 },
    {0x380f ,0xd0},
    {0x5001 ,0xa3},
    {0x5680 ,0x0 },
    {0x5681 ,0x0 },
    {0x5682 ,0xA },
    {0x5683 ,0x20},
    {0x5684 ,0x0 },
    {0x5685 ,0x0 },
    {0x5686 ,0x7 },
    {0x5687 ,0x98},
    {0xffff, 0xff},
};


//1600x1200 UXGA
static const struct regval_list OV5640_QSXGA2UXGA[] __attribute__((unused)) =
{
    {0x3800 ,0x00},
    {0x3801 ,0x00},
    {0x3802 ,0x00},
    {0x3803 ,0x00},
    {0x3804 ,0xA },
    {0x3805 ,0x3f},
    {0x3806 ,0x7 },
    {0x3807 ,0x9f},
    {0x3808 ,0x6 },
    {0x3809 ,0x40},
    {0x380a ,0x4 },
    {0x380b ,0xb0},
    {0x380c ,0xc },
    {0x380d ,0x80},
    {0x380e ,0x7 },
    {0x380f ,0xd0},
    {0x5001 ,0xa3},
    {0x5680 ,0x0 },
    {0x5681 ,0x0 },
    {0x5682 ,0xA },
    {0x5683 ,0x20},
    {0x5684 ,0x0 },
    {0x5685 ,0x0 },
    {0x5686 ,0x7 },
    {0x5687 ,0x98},
    {0xffff, 0xff},
};

//1024x768 XGA
static const struct regval_list OV5640_QSXGA2XGA[] __attribute__((unused)) =
{
    {0x3800 ,0x00},
    {0x3801 ,0x00},
    {0x3802 ,0x00},
    {0x3803 ,0x00},
    {0x3804 ,0xA },
    {0x3805 ,0x3f},
    {0x3806 ,0x7 },
    {0x3807 ,0x9f},
    {0x3808 ,0x4 },
    {0x3809 ,0x0 },
    {0x380a ,0x3 },
    {0x380b ,0x0 },
    {0x380c ,0xc },
    {0x380d ,0x80},
    {0x380e ,0x7 },
    {0x380f ,0xd0},
    {0x5001 ,0xa3},
    {0x5680 ,0x0 },
    {0x5681 ,0x0 },
    {0x5682 ,0xA },
    {0x5683 ,0x20},
    {0x5684 ,0x0 },
    {0x5685 ,0x0 },
    {0x5686 ,0x7 },
    {0x5687 ,0x98},
    {0xffff, 0xff},
};


static const struct regval_list OV5640YUV_Sensor_Dvp_Init_RGB[] =
{

     {0x3103, 0x11}, // --init code from linux driver
     {0x3008, 0x82}, // --software reset
     {0x3008, 0x42}, // --out of reset, but power down
     {0x3103, 0x02}, // --system clock from PLL
     {0x3031, 0x08}, // --bypass Vcore reg as we're feeding in 1.5V
     {0x3017, 0xff}, // --set all pins to output
     {0x3018, 0xff}, // --set all pins to output
     {0x4740, 0x26}, // --set hsync/vsync polarity to active high/active high
     {0x460c, 0x22}, // DVP PCLK divider controlled by 0x3824
     {0x3824, 0x02}, // pclk divider = 2


//     {0x3039, 0x80}, // --bypass PLL
//     {0x3108, 0x01}, // --set SCLK dividers

 //    {0x503D, 0x80}, //test mode 8 color bar vertical


     {0x3630, 0x2e}, // --mystery, likely AF coil related
     {0x3632, 0xe2}, // --as above
     {0x3633, 0x23}, // --as above
     {0x3621, 0xe0}, // --as above
     {0x3704, 0xa0}, // --begin completely mysterious registers
     {0x3703, 0x5a}, //
     {0x3715, 0x78}, //
     {0x3717, 0x01}, //
     {0x370b, 0x60}, //
     {0x3705, 0x1a}, //
     {0x3905, 0x02}, //
     {0x3906, 0x10}, //
     {0x3901, 0x0a}, //
     {0x3731, 0x12}, //
     {0x3600, 0x08}, // --marked as debug for AF coil
     {0x3601, 0x33}, // --as above
     {0x3618, 0x00}, //
     {0x3612, 0x49}, //
     {0x371b, 0x20}, //
     {0x471c, 0x50}, //
     {0x4713, 0x03}, //
     {0x3635, 0x1c}, // --more mystery registers
     {0x3634, 0x40}, //
     {0x3622, 0x01}, //

#ifdef mode_320x240
     {0x380c, 0x07}, // --total horizontal size MSB
     {0x380d, 0x68}, // --total horizontal size LSB
     {0x380e, 0x01}, // --total vertical size MSB
     {0x380f, 0xf0}, // --total vertical size LSB

     {0x3814, 0x71}, // --x inc, copied from linux driver
     {0x3815, 0x35}, // --y inc, copied from linux driver

     {0x3821, 0x01}, // --enable horizontal binning (vertical on by default)

     {0x3808, 0x01}, // --width MSB, 320
     {0x3809, 0x40}, // --width LSB
     {0x380a, 0x00}, // --height MSB, 240
     {0x380b, 0xF0}, // --height LSB

     {0x3811, 0x08}, //ISP horizontal offset low byte
     {0x3813, 0x02}, //ISP vertical offset low byte
#endif
#ifdef mode_640x480
     {0x380c, 0x07}, // --total horizontal size MSB
     {0x380d, 0x68}, // --total horizontal size LSB
     {0x380e, 0x01}, // --total vertical size MSB
     {0x380f, 0xf0}, // --total vertical size LSB

     {0x3814, 0x71}, // --x inc, copied from linux driver
     {0x3815, 0x35}, // --y inc, copied from linux driver

     {0x3821, 0x01}, // --enable horizontal binning (vertical on by default)

     {0x3808, 0x02}, // --width MSB, 640
     {0x3809, 0x80}, // --width LSB
     {0x380a, 0x01}, // --height MSB, 480
     {0x380b, 0xe0}, // --height LSB

     {0x3811, 0x08}, //ISP horizontal offset low byte
     {0x3813, 0x02}, //ISP vertical offset low byte
#endif

#ifdef mode_1280x960
     {0x3800 ,0x02}, //x start 512
     {0x3801 ,0x00},
     {0x3802 ,0x02}, //y start 512
     {0x3803 ,0x00},
     {0x3804 ,0x0A}, //x end 2607
     {0x3805 ,0x2f},
     {0x3806 ,0x07}, //y end 1947
     {0x3807 ,0x9b},
     {0x3808 ,0x05}, //output width 1280
     {0x3809 ,0x00},
     {0x380a ,0x03}, //output height 960
     {0x380b ,0xc0},
     {0x380c ,0x0B}, //total H size 2844
     {0x380d ,0x1C},
     {0x380e ,0x07}, //total V size 1968
     {0x380f ,0xB0},
     {0x3810, 0x00}, //ISP H offset 0
     {0x3811, 0x00},
     {0x3812, 0x00}, //ISP V offset 0
     {0x3813, 0x00},
     {0x3814, 0x11}, // --x inc, no binning
     {0x3815, 0x11}, // --y inc, no binning
     {0x3821, 0x00}, // --disable horizontal binning
#endif

#ifdef mode_1280x960_2x2

     //2x2 binning
     {0x3800 ,0x00}, //x start 10
     {0x3801 ,0x10},
     {0x3802 ,0x00}, //y start 4
     {0x3803 ,0x04},
     {0x3804 ,0x0A}, //x end 2607
     {0x3805 ,0x2f},
     {0x3806 ,0x07}, //y end 1947
     {0x3807 ,0x9b},
     {0x3808 ,0x05}, //output width 1280
     {0x3809 ,0x00},
     {0x380a ,0x03}, //output height 960
     {0x380b ,0xc0},
     {0x380c ,0x0B}, //total H size 2844
     {0x380d ,0x1C},
     {0x380e ,0x07}, //total V size 1968
     {0x380f ,0xB0},
     {0x3810, 0x00}, //ISP H offset 8
     {0x3811, 0x08},
     {0x3812, 0x00}, //ISP V offset 6
     {0x3813, 0x06},
     {0x3814, 0x31}, // --x inc, 2x2 binning
     {0x3815, 0x31}, // --y inc, 2x2 binning
     {0x3821, 0x01}, // --enable horizontal binning (vertical on by default)

#endif
#ifdef mode_2048x1536  //broken, clock is ~96MHz
     {0x3824, 0x01}, // pclk divider = 1
     {0x3800 ,0x00}, //x start 10
     {0x3801 ,0x10},
     {0x3802 ,0x00}, //y start 4
     {0x3803 ,0x04},
     {0x3804 ,0x0A}, //x end 2607
     {0x3805 ,0x2f},
     {0x3806 ,0x07}, //y end 1947
     {0x3807 ,0x9b},
     {0x3808 ,0x08}, //output width 2048
     {0x3809 ,0x00},
     {0x380a ,0x06}, //output height 1536
     {0x380b ,0x00},
     {0x380c ,0x0B}, //total H size 2844
     {0x380d ,0x1C},
     {0x380e ,0x07}, //total V size 1968
     {0x380f ,0xB0},
     {0x3810, 0x00}, //ISP H offset 8
     {0x3811, 0x08},
     {0x3812, 0x00}, //ISP V offset 6
     {0x3813, 0x06},
     {0x3814, 0x11}, // --x inc, no binning
     {0x3815, 0x11}, // --y inc, no binning
     {0x3821, 0x00}, // --disable horizontal binning
#endif



     {0x0000, 0x00}, //not sure why these are here
     {0x0000, 0x00}, //

     {0x3708, 0x62}, //somehow these flip red/blue
     {0x3709, 0x52}, //
     {0x370c, 0x03}, //

     {0x4001, 0x02}, // --BLC start line
     {0x4004, 0x02}, // --BLC line, copied from linux driver

     {0x3000, 0x30}, // --enable everything
     {0x3002, 0x1c}, // --enable other stuff
     {0x3004, 0xcf}, // --clock enable
     {0x4300, 0x6f}, // --RGB565
     {0x501f, 0x01}, // --ISP format mux, RGB


     {0x5000, 0xa7}, // --ISP control
                       //color interpolation disable
                       //white pixel cancellation enable
                       //black pixel cancellation disable
                       //RAW GMA enable
                       //lens correction disable
     {0x5001, 0x83}, // --further ISP setup

     {0x3820, 0x46}, //flip vertical
     //{0x3821, 0x6}, //mirror

//     {0x3037, 0x13}, // --R divider 2, PRE_DIV0=3
//     {0x3036, 0x69}, // --PLL1 OP = 8MHz x 105 = 840MHz (as example)
//     {0x3035, 0x11}, //  --SDIV0 = 2, MIPI div = 2 (not needed)
//     {0x3034, 0x1A}, // -- BIT divider = 2.5, ==> PLL1 op to SCLK and PCLK divs = 840 / (2*2*2.5) = 84MHz
//     {0x3108, 0x01}, // --PCLK div = 1, SCLK div = 2, SCLK2x div = 1
//     {0x303D, 0x30}, // --PRE_DIV= /3 , R_DIV_SP= *1
//     {0x303B, 0x19}, // --Multiplier = x25
//     {0x303C, 0x11}, // --Not sure, copying example (PLL charge pump/divider)
//     {0x3103, 0x02}, // input clock from PLL

//image correction settings
		//awb manual init
		{ 0x3400, 0x06 }, //AWB R GAIN H
		{ 0x3401, 0x80 }, //AWB R GAIN L
		{ 0x3402, 0x04 }, //AWB G GAIN H
		{ 0x3403, 0x00 }, //AWB G GAIN L
		{ 0x3404, 0x06 }, //AWB B GAIN H
		{ 0x3405, 0x00 }, //AWB B GAIN L
	    //awb auto control settings
		{ 0x5180, 0xff },
		{ 0x5181, 0xf2 },
		{ 0x5182, 0x00 },
		{ 0x5183, 0x14 },
		{ 0x5184, 0x25 },
		{ 0x5185, 0x24 },
		{ 0x5186, 0x16 },
		{ 0x5187, 0x16 },
		{ 0x5188, 0x16 },
		{ 0x5189, 0x62 },
		{ 0x518a, 0x62 },
		{ 0x518b, 0xf0 },
		{ 0x518c, 0xb2 },
		{ 0x518d, 0x50 },
		{ 0x518e, 0x30 },
		{ 0x518f, 0x30 },
		{ 0x5190, 0x50 },
		{ 0x5191, 0xf8 },
		{ 0x5192, 0x04 },
		{ 0x5193, 0x70 },
		{ 0x5194, 0xf0 },
		{ 0x5195, 0xf0 },
		{ 0x5196, 0x03 },
		{ 0x5197, 0x01 },
		{ 0x5198, 0x04 },
		{ 0x5199, 0x12 },
		{ 0x519a, 0x04 },
		{ 0x519b, 0x00 },
		{ 0x519c, 0x06 },
		{ 0x519d, 0x82 },
		{ 0x519e, 0x38 },

		//color matrix
		{ 0x5381, 0x1e },
		{ 0x5382, 0x5b },
		{ 0x5383, 0x14 },
		{ 0x5384, 0x06 },
		{ 0x5385, 0x82 },
		{ 0x5386, 0x88 },
		{ 0x5387, 0x7c },
		{ 0x5388, 0x60 },
		{ 0x5389, 0x1c },
		{ 0x538a, 0x01 },
		{ 0x538b, 0x98 },
		//sharp&noise
		{ 0x5300, 0x08 },
		{ 0x5301, 0x30 },
		{ 0x5302, 0x3f },
		{ 0x5303, 0x10 },
		{ 0x5304, 0x08 },
		{ 0x5305, 0x30 },
		{ 0x5306, 0x18 },
		{ 0x5307, 0x28 },
		{ 0x5309, 0x08 },
		{ 0x530a, 0x30 },
		{ 0x530b, 0x04 },
		{ 0x530c, 0x06 },

		//gamma
		{ 0x5480, 0x01 },
		{ 0x5481, 0x06 },
		{ 0x5482, 0x12 },
		{ 0x5483, 0x24 },
		{ 0x5484, 0x4a },
		{ 0x5485, 0x58 },
		{ 0x5486, 0x65 },
		{ 0x5487, 0x72 },
		{ 0x5488, 0x7d },
		{ 0x5489, 0x88 },
		{ 0x548a, 0x92 },
		{ 0x548b, 0xa3 },
		{ 0x548c, 0xb2 },
		{ 0x548d, 0xc8 },
		{ 0x548e, 0xdd },
		{ 0x548f, 0xf0 },
		{ 0x5490, 0x15 },

		//UV adjust
		{ 0x5580, 0x06 },
		{ 0x5583, 0x40 },
		{ 0x5584, 0x20 },
		{ 0x5589, 0x10 },
		{ 0x558a, 0x00 },
		{ 0x558b, 0xf8 },

//		//lens shading
//		{ 0x5000, 0xa7 },
//		{ 0x5800, 0x20 },
//		{ 0x5801, 0x19 },
//		{ 0x5802, 0x17 },
//		{ 0x5803, 0x16 },
//		{ 0x5804, 0x18 },
//		{ 0x5805, 0x21 },
//		{ 0x5806, 0x0F },
//		{ 0x5807, 0x0A },
//		{ 0x5808, 0x07 },
//		{ 0x5809, 0x07 },
//		{ 0x580a, 0x0A },
//		{ 0x580b, 0x0C },
//		{ 0x580c, 0x0A },
//		{ 0x580d, 0x03 },
//		{ 0x580e, 0x01 },
//		{ 0x580f, 0x01 },
//		{ 0x5810, 0x03 },
//		{ 0x5811, 0x09 },
//		{ 0x5812, 0x0A },
//		{ 0x5813, 0x03 },
//		{ 0x5814, 0x01 },
//		{ 0x5815, 0x01 },
//		{ 0x5816, 0x03 },
//		{ 0x5817, 0x08 },
//		{ 0x5818, 0x10 },
//		{ 0x5819, 0x0A },
//		{ 0x581a, 0x06 },
//		{ 0x581b, 0x06 },
//		{ 0x581c, 0x08 },
//		{ 0x581d, 0x0E },
//		{ 0x581e, 0x22 },
//		{ 0x581f, 0x18 },
//		{ 0x5820, 0x13 },
//		{ 0x5821, 0x12 },
//		{ 0x5822, 0x16 },
//		{ 0x5823, 0x1E },
//		{ 0x5824, 0x64 },
//		{ 0x5825, 0x2A },
//		{ 0x5826, 0x2C },
//		{ 0x5827, 0x2A },
//		{ 0x5828, 0x46 },
//		{ 0x5829, 0x2A },
//		{ 0x582a, 0x26 },
//		{ 0x582b, 0x24 },
//		{ 0x582c, 0x26 },
//		{ 0x582d, 0x2A },
//		{ 0x582e, 0x28 },
//		{ 0x582f, 0x42 },
//		{ 0x5830, 0x40 },
//		{ 0x5831, 0x42 },
//		{ 0x5832, 0x08 },
//		{ 0x5833, 0x28 },
//		{ 0x5834, 0x26 },
//		{ 0x5835, 0x24 },
//		{ 0x5836, 0x26 },
//		{ 0x5837, 0x2A },
//		{ 0x5838, 0x44 },
//		{ 0x5839, 0x4A },
//		{ 0x583a, 0x2C },
//		{ 0x583b, 0x2a },
//		{ 0x583c, 0x46 },
//		{ 0x583d, 0xCE },

		//mode lens shading
		{ 0x5688, 0x22 },
		{ 0x5689, 0x22 },
		{ 0x568a, 0x42 },
		{ 0x568b, 0x24 },
		{ 0x568c, 0x42 },
		{ 0x568d, 0x24 },
		{ 0x568e, 0x22 },
		{ 0x568f, 0x22 },

		//automatic exposure settings settings
		{ 0x3a0f, 0x30 }, //AEC CTRL0F
		{ 0x3a10, 0x28 }, //AEC CTRL10
		{ 0x3a1b, 0x30 }, //AEC CTRL11
		{ 0x3a1e, 0x28 }, //AEC CTRL1B
		{ 0x3a11, 0x61 }, //AEC CTRL1E
		{ 0x3a1f, 0x10 }, //AEC CTRL1F

		//black level settings
		{ 0x4005, 0x1a }, //BLC always update
//		{ 0x3406, 0x00 }, //AWB auto mode
        { 0x3406, 0x01 }, //AWB manual mode
        { 0x3503, 0x00 }, //auto AEC, auto AGC
		{ 0xffff, 0xff } // end

};

static resolution_t last_res = RES_NONE;

typedef struct dmaControl {
  const stm32_dma_stream_t  *dmastp;
  TIM_TypeDef               *timer;
  uint8_t                   *capture_buffer;
  uint16_t                  dbm_index;
  volatile bool             capture;
  uint32_t                  dma_flags;
  volatile bool             dma_error;
  uint16_t                  dma_count;
} dma_capture_t;

void OV5640_SetResolution(resolution_t res)
{
   //chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Configure Resolution ");
    switch(res) {
        case RES_QQVGA:
            //chprintf((BaseSequentialStream *)&SD_console,"QQVGA\r\n");
            for(uint32_t i=0; (OV5640_QSXGA2QQVGA[i].reg != 0xffff) || (OV5640_QSXGA2QQVGA[i].val != 0xff); i++)
                I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640_QSXGA2QQVGA[i].reg, OV5640_QSXGA2QQVGA[i].val);
            break;

        case RES_QVGA:
            //chprintf((BaseSequentialStream *)&SD_console,"QVGA\r\n");
            for(uint32_t i=0; (OV5640_QSXGA2QVGA[i].reg != 0xffff) || (OV5640_QSXGA2QVGA[i].val != 0xff); i++)
                I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640_QSXGA2QVGA[i].reg, OV5640_QSXGA2QVGA[i].val);
            break;

        case RES_VGA:
            //chprintf((BaseSequentialStream *)&SD_console,"VGA\r\n");
            for(uint32_t i=0; (OV5640_QSXGA2VGA[i].reg != 0xffff) || (OV5640_QSXGA2VGA[i].val != 0xff); i++)
                I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640_QSXGA2VGA[i].reg, OV5640_QSXGA2VGA[i].val);
            break;

        case RES_XGA:
          //chprintf((BaseSequentialStream *)&SD_console,"XGA\r\n");
            for(uint32_t i=0; (OV5640_QSXGA2XGA[i].reg != 0xffff) || (OV5640_QSXGA2XGA[i].val != 0xff); i++)
                I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640_QSXGA2XGA[i].reg, OV5640_QSXGA2XGA[i].val);
            break;

        case RES_UXGA:
          //chprintf((BaseSequentialStream *)&SD_console,"UXGA\r\n");
            for(uint32_t i=0; (OV5640_QSXGA2UXGA[i].reg != 0xffff) || (OV5640_QSXGA2UXGA[i].val != 0xff); i++)
                I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640_QSXGA2UXGA[i].reg, OV5640_QSXGA2UXGA[i].val);
            break;

        case RES_5MP:
          //chprintf((BaseSequentialStream *)&SD_console,"5MP\r\n");
            for(uint32_t i=0; (OV5640_5MP_JPEG[i].reg != 0xffff) || (OV5640_5MP_JPEG[i].val != 0xff); i++)
                I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640_5MP_JPEG[i].reg, OV5640_5MP_JPEG[i].val);
            break;

        case RES_NONE: // No configuration is made
          //chprintf((BaseSequentialStream *)&SD_console,"none\r\n");
            break;

        default: // Default QVGA
          //chprintf((BaseSequentialStream *)&SD_console,"QVGA\r\n");
            for(uint32_t i=0; (OV5640_QSXGA2QVGA[i].reg != 0xffff) || (OV5640_QSXGA2QVGA[i].val != 0xff); i++)
                I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640_QSXGA2QVGA[i].reg, OV5640_QSXGA2QVGA[i].val);
    }
}


/**
  * Captures an image from the camera.
  * resolution set by #define mode_xxxx from ov5640.h
  */
uint32_t OV5640_Snapshot2RAM_RGB() {

  palSetPad(GPIOD, GPIOD_LED_RED);  //turn on red led on discovery

  //    // Set resolution (if not already done earlier)
  //  if(res != last_res) {
  //      if(res == RES_MAX) {
  //          OV5640_SetResolution(RES_UXGA); // FIXME: We actually have to choose the resolution which fits in the memory
  //      } else {
  //          OV5640_SetResolution(res);
  //      }
  //  }

  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Start RGB Capture\r\n");
  // DCMI init and start capture
  OV5640_InitDCMI_RGB();

  // Wait for capture to start
  samplingFinished = false;
  systime_t timeout = chVTGetSystemTimeX() + TIME_MS2I(1000); // Timeout 1sec
  while(!samplingFinished && chVTGetSystemTimeX() < timeout)
      chThdSleepMilliseconds(1);
  if(chVTGetSystemTimeX() >= timeout) { // Timeout has occurred
    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > No image captured\r\n");
      return false;
  } else { // Data successfully sampled
    palClearPad(GPIOD, GPIOD_LED_RED); //turn off red led on discovery
    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Capture Successful\r\n");
  }

  return true;
}

/**
  * Captures an image from the camera.
  * @buffer Buffer in which the image can be sampled
  * @size Size of buffer
  * @res Resolution of the image
  * If resolution MAX_RES has been chosen, the maximum resolution will be
  * chosen for the available buffer. Due to the JPEG compression
  * that could lead to different resolutions on different method calls.
  * The method returns the size of the image.
  */
uint32_t OV5640_Snapshot2RAM() {

  int samplingFinished_DCMI_controller = 0;

  //    // Set resolution (if not already done earlier)
  //  if(res != last_res) {
  //      if(res == RES_MAX) {
  //          OV5640_SetResolution(RES_UXGA); // FIXME: We actually have to choose the resolution which fits in the memory
  //      } else {
  //          OV5640_SetResolution(res);
  //      }
  //  }

  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Start JPEG Capture\r\n");
  // DCMI init and start capture
  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Start DCMI capture...\r\n");

  //clear image buffer
  memset(OV5640_ram_buffer, 0x00, sizeof(OV5640_ram_buffer));

  //start transfer
  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Init DMA\r\n");
  OV5640_InitDMA();
  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Init DCMI\r\n");
  OV5640_InitDCMI();

  // Wait for capture to start
  systime_t timeout = chVTGetSystemTimeX() + TIME_MS2I(5000); // Timeout 5sec

  samplingFinished_DCMI_controller = !(DCMI->CR &1); //Capture bit
  while(!samplingFinished_DCMI_controller && chVTGetSystemTimeX() < timeout){
    chThdSleepMilliseconds(1);
    samplingFinished_DCMI_controller = !(DCMI->CR &1); //Capture bit
    ////chprintf((BaseSequentialStream *)&SD_console,"%d %d %d\r\n", DMA2_Stream1->NDTR, (DCMI->CR)&1, DCMI->SR);
  }
  dcmi_dma_open_ended_stop();
  OV5640_ram_buffer_length = dcmi_dma_bytes_received;
  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > DCMI transfer done, %d bytes captured\r\n", dcmi_dma_bytes_received);

  if(!samplingFinished_DCMI_controller) { // Timeout has occurred
    palSetPad(GPIOD, GPIOD_LED_RED);  //turn on red led on discovery
    memcpy( OV5640_ram_buffer, noCameraFound, sizeof(noCameraFound));
    OV5640_ram_buffer_length = sizeof(noCameraFound);
    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > No image captured\r\n");
      return false;
  } else { // Data successfully sampled

    //clean buffer data off end of file
    //loop back from end of buffer until first nonzero value is found
    //EOF for jpeg is ... 0xff 0xd9
    while(OV5640_ram_buffer_length > 0){
      if(OV5640_ram_buffer[OV5640_ram_buffer_length-1] == 0){
        OV5640_ram_buffer_length = OV5640_ram_buffer_length - 1;
      } else{
        break;
      }
    }
    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Capture successful, %d byte image data\r\n", OV5640_ram_buffer_length);
  }

  return true;
}

uint32_t OV5640_Snapshot2SD(char* filename) {
	FIL fil;
	FRESULT fr;
	fr = f_open(&fil, filename, FA_WRITE | FA_CREATE_NEW);
	if (fr != 0) return fr;
 
	uint32_t snapshot = OV5640_Snapshot2RAM();
	if (!snapshot) return 1;
 
	UINT bytes_written;
	fr = f_write(&fil, OV5640_ram_buffer, OV5640_ram_buffer_length, &bytes_written);
	
	if (fr != 0) return fr;
	// return 20 because the first 19 are file result errors
	if (bytes_written < OV5640_ram_buffer_length) return 20;
	fr = f_close(&fil);
	return 0;
}
	


/*
 * Other drivers using resources that can cause DMA competition are locked.
 */
msg_t OV5640_LockResourcesForCapture(void) {
//  /* TODO: have to make this a loop which would handle multiple receivers. */
//
//  /* Acquire radio after any active TX completes. */
//  msg_t msg = pktLockRadioTransmit(PKT_RADIO_1, TIME_INFINITE);
//  if(msg != MSG_OK) {
//    return msg;
//  }
//  if(pktIsReceiveActive(PKT_RADIO_1)) {
//    pktLLDradioPauseDecoding(PKT_RADIO_1);
//    decode_pause = true;
//  } else
//    decode_pause = false;
//  I2C_Lock();
//
//  //pktLLDradioPauseDecoding(PKT_RADIO_1);
//  //pktPauseDecoding(PKT_RADIO_1);
//  /* Hold TRACE output on USB. */
///*  if(isUSBactive())
//    chMtxLock(&trace_mtx);*/
  return MSG_OK;
}

/*
 * Unlock competing drivers.
 */
void OV5640_UnlockResourcesForCapture(void) {
//  /* Re-enable TRACE output on USB. */
///*  if(isUSBactive())
//    chMtxUnlock(&trace_mtx);*/
//  I2C_Unlock();
//  /* TODO: have to make this a loop which would handle multiple receivers. */
//  if(pktIsReceivePaused(PKT_RADIO_1) && decode_pause) {
//    pktLLDradioResumeDecoding(PKT_RADIO_1);
//  }
//  //pktResumeDecoding(PKT_RADIO_1);
//  /* Enable TX tasks to run. */
//  pktUnlockRadioTransmit(PKT_RADIO_1);
}

/**
 *
 */
uint32_t OV5640_Capture() {

    /* WARNING: Do not use TRACE between locking and unlocking. */
	if(OV5640_LockResourcesForCapture() != MSG_OK) {
	  /* Unable to lock resources. */
	  return 0;
	}

	OV5640_UnlockResourcesForCapture();

    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Capture success\r\n");

	error = 0x0;
	return error;
}

/**
  * Initializes GPIO for OV5640 pseudo DCMI
  */
void OV5640_InitGPIO(void)
{
  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Activate XCLK\r\n");
  palSetPadMode(GPIOA, 8, PAL_MODE_ALTERNATE(0)); // PA8             -> XCLK
  //set up in boards.h
  //    palSetPadMode(GPIOA, GPIOA_DCMI_HSYNC, PAL_MODE_ALTERNATE(13)); // HSYNC -> PA4
  //    palSetPadMode(GPIOA, GPIOA_DCMI_PXCLK, PAL_MODE_ALTERNATE(13)); // PCLK  -> PA6
  //    palSetPadMode(GPIOG, GPIOG_DCMI_VSYNC, PAL_MODE_ALTERNATE(13)); // VSYNC -> PG9
  //    palSetPadMode(GPIOC, GPIOC_DCMI_D0, PAL_MODE_ALTERNATE(13)); // D0    -> PC6
  //    palSetPadMode(GPIOC, GPIOC_DCMI_D1, PAL_MODE_ALTERNATE(13)); // D1    -> PC7
  //    palSetPadMode(GPIOG, GPIOG_DCMI_D2, PAL_MODE_ALTERNATE(13)); // D2    -> PG10
  //    palSetPadMode(GPIOG, GPIOG_DCMI_D3, PAL_MODE_ALTERNATE(13)); // D3    -> PG12
  //    palSetPadMode(GPIOE, GPIOE_DCMI_D4, PAL_MODE_ALTERNATE(13)); // D4    -> PE4
  //    palSetPadMode(GPIOD, GPIOD_DCMI_D5, PAL_MODE_ALTERNATE(13)); // D5    -> PD3
  //    palSetPadMode(GPIOE, GPIOE_DCMI_D6, PAL_MODE_ALTERNATE(13)); // D6    -> PE5
  //    palSetPadMode(GPIOE, GPIOE_DCMI_D7, PAL_MODE_ALTERNATE(13)); // D7    -> PE6

	palSetLineMode(LINE_CAM_EN, PAL_MODE_OUTPUT_PUSHPULL);
	palSetLineMode(LINE_CAM_RESET, PAL_MODE_OUTPUT_PUSHPULL);
	/* Force reset low prior to power up. */
	palClearLine(LINE_CAM_RESET);
	chThdSleep(TIME_MS2I(10));
}

void OV5640_TransmitConfig_RGB(void)
{
    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Software reset\r\n");
    I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3103, 0x11);
    I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3008, 0x82);
    chThdSleep(TIME_MS2I(100));

    ////RGB 565 mode
    //  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Configure RGB565 640x480\r\n");
    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Configure RGB565 %dx%d\r\n",OV5640_MAXX,OV5640_MAXY);
    for(uint32_t i=0; (OV5640YUV_Sensor_Dvp_Init_RGB[i].reg != 0xffff) || (OV5640YUV_Sensor_Dvp_Init_RGB[i].val != 0xff); i++)
        I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640YUV_Sensor_Dvp_Init_RGB[i].reg, OV5640YUV_Sensor_Dvp_Init_RGB[i].val);

    chThdSleep(TIME_MS2I(500));

//  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Light Mode: Auto\r\n");
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3406, 0x00);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3400, 0x04);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3401, 0x00);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3402, 0x04);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3403, 0x00);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3404, 0x04);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3405, 0x00);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x13); // end group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0xa3); // lanuch group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5183, 0x0 );
//
//  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Saturation: 0\r\n");
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5381, 0x1c);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5382, 0x5a);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5383, 0x06);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5384, 0x1a);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5385, 0x66);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5386, 0x80);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5387, 0x82);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5388, 0x80);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5389, 0x02);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x538b, 0x98);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x538a, 0x01);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x13); // end group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0xa3); // launch group 3
//
//  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Brightness: 0\r\n");
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5587, 0x00);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5588, 0x01);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x13); // end group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0xa3); // launch group 3
//
//  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Contrast: 0\r\n");
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5586, 0x20);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5585, 0x00);
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x13); // end group 3
//  I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0xa3); // launch group 3
}

void OV5640_TransmitConfig(void)
{
	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Software reset\r\n");
	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3103, 0x11);
	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3008, 0x82);
	chThdSleep(TIME_MS2I(100));

	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Initialization\r\n");
	for(uint32_t i=0; (OV5640YUV_Sensor_Dvp_Init[i].reg != 0xffff) || (OV5640YUV_Sensor_Dvp_Init[i].val != 0xff); i++)
	  I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640YUV_Sensor_Dvp_Init[i].reg, OV5640YUV_Sensor_Dvp_Init[i].val);

	chThdSleep(TIME_MS2I(500));

//	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Configure JPEG QSXGA\r\n");
//	for(uint32_t i=0; (OV5640_JPEG_QSXGA[i].reg != 0xffff) || (OV5640_JPEG_QSXGA[i].val != 0xff); i++){
//	  I2C_write8_16bitreg(OV5640_I2C_ADR, OV5640_JPEG_QSXGA[i].reg, OV5640_JPEG_QSXGA[i].val);
//	}

	//	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Light Mode: Auto\r\n");
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3406, 0x00);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3400, 0x04);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3401, 0x00);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3402, 0x04);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3403, 0x00);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3404, 0x04);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3405, 0x00);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x13); // end group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0xa3); // lanuch group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5183, 0x0 );
//
//	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Saturation: 0\r\n");
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5381, 0x1c);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5382, 0x5a);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5383, 0x06);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5384, 0x1a);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5385, 0x66);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5386, 0x80);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5387, 0x82);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5388, 0x80);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5389, 0x02);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x538b, 0x98);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x538a, 0x01);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x13); // end group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0xa3); // launch group 3
//
//	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Brightness: 0\r\n");
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5587, 0x00);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5588, 0x01);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x13); // end group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0xa3); // launch group 3
//
//	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > ... Contrast: 0\r\n");
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x03); // start group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5586, 0x20);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x5585, 0x00);
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0x13); // end group 3
//	I2C_write8_16bitreg(OV5640_I2C_ADR, 0x3212, 0xa3); // launch group 3
}

void OV5640_powerup(void) {
  // Configure pins
    OV5640_InitGPIO();

    // Switch on camera
    palClearLine(LINE_CAM_EN);        // Switch on camera
    chThdSleep(TIME_MS2I(5));       // Spec is >= 1ms delay after DOVDD stable
    palSetLine(LINE_CAM_RESET);     // De-assert reset

    chThdSleep(TIME_MS2I(50));     // Spec is >= 20ms delay after reset high to SCCB ready
}


void OV5640_init(void)
{
	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > Init pins\r\n");
	OV5640_powerup();
//	OV5640_setLightIntensity();

	// Send settings to OV5640
	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > Transmit config to camera\r\n");
	OV5640_TransmitConfig();

	chThdSleep(TIME_MS2I(200));

    // DCMI DMA - done in OV5640_Snapshot2RAM()
   // //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Init DMA\r\n");
   // OV5640_InitDMA();

    // DCMI Init - done in OV5640_Snapshot2RAM()
   // //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Init DCMI\r\n");
   // OV5640_InitDCMI();
}

void OV5640_init_RGB(void)
{
    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Init pins\r\n");
    OV5640_powerup();
    //OV5640_setLightIntensity();

    // Send settings to OV5640
    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Transmit config to camera\r\n");
    OV5640_TransmitConfig_RGB();

    chThdSleep(TIME_MS2I(200));

    // DCMI DMA
    //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Init DMA\r\n");
    OV5640_InitDMA_RGB();

    // DCMI Init - done in OV5640_Snapshot2RAM()
   // //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Init DCMI\r\n");
   // OV5640_InitDCMI();
}

void OV5640_deinit(void)
{

  // DCMI Init
  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Deinit DMA\r\n");
  OV5640_DeinitDCMI();

  // DCMI DMA
  //chprintf((BaseSequentialStream *)&SD_console,"CAM  > Deinit DCMI\r\n");
  OV5640_DeinitDMA();

	// Power off OV5640
	//chprintf((BaseSequentialStream *)&SD_console,"CAM  > Switch off\r\n");

//	palSetLineMode(LINE_CAM_PCLK, PAL_MODE_INPUT);
//	palSetLineMode(LINE_CAM_VSYNC, PAL_MODE_INPUT);
//
//	palSetLineMode(LINE_CAM_XCLK, PAL_MODE_INPUT);
//	palSetLineMode(LINE_CAM_D2, PAL_MODE_INPUT);
//	palSetLineMode(LINE_CAM_D3, PAL_MODE_INPUT);
//	palSetLineMode(LINE_CAM_D4, PAL_MODE_INPUT);
//	palSetLineMode(LINE_CAM_D5, PAL_MODE_INPUT);
//	palSetLineMode(LINE_CAM_D6, PAL_MODE_INPUT);
//	palSetLineMode(LINE_CAM_D7, PAL_MODE_INPUT);
//	palSetLineMode(LINE_CAM_D8, PAL_MODE_INPUT);
//	palSetLineMode(LINE_CAM_D9, PAL_MODE_INPUT);
//
	palSetLineMode(LINE_CAM_EN, PAL_MODE_INPUT);
	palSetLineMode(LINE_CAM_RESET, PAL_MODE_INPUT);

	last_res = RES_NONE;
}


/**
  * Initializes DCMI
  */
void OV5640_InitDCMI_RGB(void)
{
    // Clock enable
    RCC->AHB2ENR |= RCC_AHB2Periph_DCMI;

    // Configure DCMI
    DCMI->CR &= ~((uint32_t)DCMI_CR_CM     | DCMI_CR_ESS   | DCMI_CR_PCKPOL |
                DCMI_CR_HSPOL  | DCMI_CR_VSPOL | DCMI_CR_FCRC_0 |
                DCMI_CR_FCRC_1 | DCMI_CR_EDM_0 | DCMI_CR_EDM_1);
//    DCMI->CR =  DCMI_CaptureMode_SnapShot | DCMI_SynchroMode_Hardware | DCMI_PCKPolarity_Falling |
//                DCMI_VSPolarity_High | DCMI_HSPolarity_High | DCMI_CaptureRate_All_Frame | DCMI_ExtendedDataMode_8b;
    DCMI->CR =  DCMI_CaptureMode_SnapShot | DCMI_SynchroMode_Hardware | DCMI_PCKPolarity_Rising |
                DCMI_VSPolarity_High | DCMI_HSPolarity_High | DCMI_CaptureRate_All_Frame | DCMI_ExtendedDataMode_8b;
    // DCMI enable
    DCMI->CR |= (uint32_t)DCMI_CR_ENABLE;
    // Capture enable
    DCMI->CR |= (uint32_t)DCMI_CR_CAPTURE;
}

/**
  * Initializes DCMI
  */
void OV5640_InitDCMI(void)
{
    // Clock enable
    RCC->AHB2ENR |= RCC_AHB2Periph_DCMI;

    // Configure DCMI
    DCMI->CR &= ~((uint32_t)DCMI_CR_CM | DCMI_CR_JPEG | DCMI_CR_ESS   | DCMI_CR_PCKPOL |
                DCMI_CR_HSPOL  | DCMI_CR_VSPOL | DCMI_CR_FCRC_0 |
                DCMI_CR_FCRC_1 | DCMI_CR_EDM_0 | DCMI_CR_EDM_1);
//    DCMI->CR =  DCMI_CaptureMode_SnapShot | DCMI_SynchroMode_Hardware | DCMI_PCKPolarity_Falling |
//                DCMI_VSPolarity_High | DCMI_HSPolarity_High | DCMI_CaptureRate_All_Frame | DCMI_ExtendedDataMode_8b;
//    DCMI->CR =  DCMI_CaptureMode_SnapShot | DCMI_SynchroMode_Hardware | DCMI_PCKPolarity_Rising |
//                DCMI_VSPolarity_High | DCMI_HSPolarity_High | DCMI_CaptureRate_All_Frame | DCMI_ExtendedDataMode_8b;
    DCMI->CR =  DCMI_CaptureMode_SnapShot | DCMI_CR_JPEG | DCMI_SynchroMode_Hardware | DCMI_PCKPolarity_Rising |
                DCMI_VSPolarity_High | DCMI_HSPolarity_Low | DCMI_CaptureRate_All_Frame | DCMI_ExtendedDataMode_8b;
    // DCMI enable
    DCMI->CR |= (uint32_t)DCMI_CR_ENABLE;
    // Capture enable
    DCMI->CR |= (uint32_t)DCMI_CR_CAPTURE;
}

void OV5640_DeinitDCMI(void)
{
    // Clock enable
    RCC->AHB2ENR &= ~RCC_AHB2Periph_DCMI;
}

bool OV5640_isAvailable(void) {

  OV5640_powerup();

  uint8_t val, val2;
  bool ret;
  if(I2C_read8_16bitreg(OV5640_I2C_ADR, 0x300A, &val)
      && I2C_read8_16bitreg(OV5640_I2C_ADR, 0x300B, &val2)) {
      ret = val == 0x56 && val2 == 0x40;
  } else {
      error = 0x1;
      ret = false;
  }

  // Switch off camera
  OV5640_deinit();

  return ret;
}

void OV5640_DeinitDMA(void)
{
    const stm32_dma_stream_t *stream = STM32_DMA2_STREAM1;
    dmaStreamDisable(stream);
}

/**
  * Initializes DMA
  */
void OV5640_InitDMA_RGB(void)
{
    dcmi_dma_buffer = (uintptr_t) OV5640_ram_buffer;
    dcmi_dma_image_size = OV5640_NUM_PIXELS*sizeof(uint16_t);
    dcmi_dma_done_callback = dma_avail;
    dcmi_dma_error_callback = dma_error;
    dcmi_dma_init();
    dcmi_dma_arm();
}

/**
  * Initializes DMA
  */
void OV5640_InitDMA(void)
{
    dcmi_dma_buffer = (uintptr_t) OV5640_ram_buffer;
    dcmi_dma_buffer_size = OV5640_NUM_PIXELS*sizeof(uint16_t);
    dcmi_dma_done_callback = dma_avail;
    dcmi_dma_error_callback = dma_error;
    dcmi_dma_init_open_ended();
    dcmi_dma_arm();
}

/**
  * ISR callback called when DCMI buffer moved into RAM by DMA completly
  * Buffer size: OV9655_NUM_PIXELS = OV9655_MAXX * 16 * bytes
  * Double buffer mode is activated.
  */
void dma_avail(void*p, uint32_t flags)
{
    (void)flags; (void)p;
    samplingFinished = true;
}

void dma_error(void*p, uint32_t flags)
{
    (void)flags; (void)p;
    osalDbgAssert(FALSE, "CAM  > dma error\r\n");
}

void OV5640_setLightIntensity(void)
{
	uint8_t val1,val2,val3;
	I2C_read8_16bitreg(OV5640_I2C_ADR, 0x3C1B, &val1);
	I2C_read8_16bitreg(OV5640_I2C_ADR, 0x3C1C, &val2);
	I2C_read8_16bitreg(OV5640_I2C_ADR, 0x3C1D, &val3);
	lightIntensity = (val1 << 16) | (val2 << 8) | val3;
}

uint32_t OV5640_getLastLightIntensity(void)
{
	uint32_t ret = lightIntensity;
	return ret;
}

uint8_t OV5640_hasError(void)
{
	return error;
}

